...
import { HTTPReportsPurchasingDebt } from "../../../../../apis/User/reports/purchase";
...
const UmurHutangTable = () => {
    ...
    const DetailData = (item: any) => {
        dispatch(setDebtId({ data: { id: item.supplierId, name: item.supplierName } }))
        navigate('/laporan/transaksi/pembelian/hutang/detail')
    }

    const GetDebtReport = async () => {
        setLoader(true)
        try {
            const resp = await HTTPReportsPurchasingDebt({
