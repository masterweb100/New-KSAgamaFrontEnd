...
import { HTTPReportsSalesCustomer, HTTPReportsSalesCustomerByID } from '../../../../../apis/User/reports/sale';
...
const PendapatanPelangganTable = () => {
    ...
    const GetCustomerReports = async () => {
        setLoader(true)
        try {
            const resp = await HTTPReportsSalesCustomer({
    ...
    React.useEffect(() => { GetCustomerReports() }, [init])

    const GetCustomerProducts = async (customerId: string) => {
        setChildLoader(true)
        try {
            const resp = await HTTPReportsSalesCustomerByID({
            ...
            <Box component="main" sx={{ bgcolor: '#f4f5ff', p: isMobile ? 2 : 5, width: '100vw', minHeight: '100vh' }}>
                ...
                <div style={{ maxWidth: isMobile ? '100vw' : '78vw' }}>
                    ...
                    <Stack direction={isMobile ? 'column' : 'row'} gap={3} alignItems={isMobile ? 'flex-start' : 'center'} justifyContent={'space-between'} sx={{ marginTop: 3 }}>
                        <h2 style={{ margin: 0, width: '100%' }}>Pendapatan Per Pelanggan</h2>
                                                            ...
                                                            return (
                                                                <TableBody key={index}>
                                                                    <TableRow
                                                                        ...
                                                                        onClick={() => handleChangePanel(index, item.customerId)}
                                                                    >
                                                                    ...
                                                                    </TableRow>
                                                                    {
                                                                        expanded[0] === index && expanded[1] === true ?
                                                                            <TableRow>
                                                                                ...
                                                                                >
                                                                                    <PendapatanPelangganChildTable data={SelectedData} loader={childLoader} />
                                                                                </StyledTableCell>
                                                                            </TableRow>
                                                                            :
                                                                            null
