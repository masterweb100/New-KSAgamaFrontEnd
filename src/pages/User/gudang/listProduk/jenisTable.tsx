...
import { HTTPDeleteTypes, HTTPGetTypes, HTTPUpdateType } from "../../../../apis/User/product/types";
...
if (error.status === 500) {
    ...
    const handleSelectAllClick = (event: React.ChangeEvent<HTMLInputElement>) => {
        if (event.target.checked) {
            const newSelected = TypesData.map((item: any, index: number) => item.id);
            setSelected(newSelected);
            return;
        }
        setSelected([]);
    ...
    const FormAddJenis = () => {
        if (BrandData.length > 0) {
            navigate('/gudang/list-produk/form-jenis/add')
        }
    }
    ...
    const onEdit = async () => {
        setSend(true)
        try {
            await HTTPUpdateType({
                typeName: itemSelected.typeName,
                id: itemSelected.id,
    ...
    return (
        <div>
            <Stack direction={'row'} justifyContent={'space-between'}>
                <Tooltip title={'Brand Produk belum tersedia'} placement="right" open={tooltipOpen} onOpen={handleOpen} onClose={() => setTooltipOpen(false)}>
                    <div onClick={FormAddJenis} style={{ ...CENTER, backgroundColor: BrandData.length === 0 ? "#ababab" : Colors.primary, borderRadius: 5, cursor: 'pointer', padding: isMobile ? '12px 15px' : '10px 30px', alignSelf: 'flex-start' }}>
            ...
            >
                <Stack alignItems={"center"} gap={2} direction={"row"}>
                    ...
                    <p style={{ color: "#fff", fontWeight: 500, margin: 0 }}>Daftar Data Jenis Produk</p>
                </Stack>
                <TextField
                    type="search"
                    size="small"
            ...
            <Dialog open={editModal} onClose={handleEdit} PaperProps={{ style: { maxWidth: '100vw' } }}>
                <DialogTitle>
                    <Stack direction={'row'} width={'100%'} alignItems={'center'} justifyContent={'space-between'}>
                        <strong><span>Form Edit Data Jenis</span></strong>
                ...
                <DialogContent>
                    <Stack direction={"column"} gap={1}>
                        ...
                        <TextField
                            ...
                            value={itemSelected.typeName}
                            onChange={(e) => setItemSelected({ ...itemSelected, typeName: e.target.value })}
                        />
                    </Stack>
                    <Stack
                        direction={"row"}
