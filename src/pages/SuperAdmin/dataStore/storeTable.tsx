...
const StoreTable = (props: any) => {
  ...
  const handleSelectAllClick = (event: React.ChangeEvent<HTMLInputElement>) => {
    if (event.target.checked) {
      const newSelected = props.data.map((n: any, index: number) => n.id.toString());
      setSelected(newSelected);
      return;
    }
    setSelected([]);
  ...
  const FormUpdateStore = (item: any) => {
  ...
  return (
    <div>
      <Stack direction={"row"} justifyContent={"space-between"}>
        {
          DataUser.length === 0 ?
            <Tooltip title="Pengguna belum tersedia" placement="right">
              <div
                style={{
                  ...CENTER,
